...
chcp 65001 >nul
title QuantScope 启动器

echo ========================================
echo   QuantScope 绿色版启动器
echo ========================================
echo.
...
chcp 65001 >nul
title QuantScope 停止器

echo ========================================
echo   QuantScope 绿色版停止器
echo ========================================
echo.
...
; QuantScope Windows 安装器脚本

!include "MUI2.nsh"

; 基本信息
Name "QuantScope"
OutFile "QuantScope-Setup.exe"
InstallDir "$PROGRAMFILES64\QuantScope"
...
Section "主程序" SecMain
    ...
    ; 创建快捷方式
    CreateDirectory "$SMPROGRAMS\QuantScope"
    CreateShortcut "$SMPROGRAMS\QuantScope\QuantScope.lnk" "$INSTDIR\start.bat"
    CreateShortcut "$DESKTOP\QuantScope.lnk" "$INSTDIR\start.bat"
    
    ; 写入卸载信息
    WriteUninstaller "$INSTDIR\Uninstall.exe"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\QuantScope" \
                     "DisplayName" "QuantScope"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\QuantScope" \
...
Section "Uninstall"
    ...
    ; 删除快捷方式
    Delete "$DESKTOP\QuantScope.lnk"
    RMDir /r "$SMPROGRAMS\QuantScope"
    
    ; 删除注册表
    DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\QuantScope"
SectionEnd
```
...
def build_installer():
    ...
    if result.returncode == 0:
        print("✅ 安装器构建成功！")
        print(f"📦 输出文件: QuantScope-Setup.exe")
    else:
        print("❌ 安装器构建失败")
        print(result.stderr)
        sys.exit(1)
