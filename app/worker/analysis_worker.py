...
class AnalysisWorker:
    ...
    async def _process_task(self, task_data: Dict[str, Any]):
        ...
        try:
            ...
            lease_task = asyncio.create_task(self._lease_renewal_loop(task_id))
    ...
    async def _lease_renewal_loop(self, task_id: str):
        """后台续期任务租约，避免长时间分析被重复消费。"""
        renewal_interval = max(5.0, float(self.queue_service.visibility_timeout) / 3)

        while self.running and self.current_task == task_id:
            try:
                await asyncio.sleep(renewal_interval)

                if not self.running or self.current_task != task_id:
                    break

                renewed = await self.queue_service.renew_task_lease(task_id, self.worker_id)
                if not renewed:
                    logger.warning(f"任务租约续期失败，可能已失去所有权: {task_id}")
                    break

                logger.debug(f"任务租约续期成功: {task_id}")
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error(f"任务租约续期异常: {task_id} - {e}")
