...
from app.models.analysis import (AnalysisParameters, AnalysisStatus,
                                 AnalysisTask, SingleAnalysisRequest)
        ...
        if doc and "llm_configs" in doc:
            ...
            for config_dict in llm_configs:
                if config_dict.get("model_name") == model_name:
                    ...
                    api_key = None
                    if _is_valid_api_key(model_api_key):
                    ...
                    backend_url = _normalize_provider_backend_url(provider, backend_url)
                    client.close()
                    return {
                        "provider": provider,
...
def _normalize_provider_backend_url(provider: str, backend_url: Optional[str]) -> Optional[str]:
    """规范厂商端点，避免错误接口。"""
    if not backend_url:
        return backend_url
    normalized = backend_url.strip().rstrip("/")
    if (provider or "").lower() in {"dashscope", "qwen"} and normalized.endswith("/api/v1"):
...
class SimpleAnalysisService:
    ...
    def _run_analysis_sync(
        ...
        try:
            ...
            # 定义进度回调函数，用于接收 AgentScope 的实时进度
