...
class AnalysisService:
    ...
    def _get_trading_graph(self, config: Dict[str, Any]) -> TradingAgentsGraph:
        """每个任务创建独立的 TradingAgentsGraph，避免实例级状态串扰。"""
        logger.info(f"创建独立TradingAgents实例: {config.get('llm_provider', 'default')}")
    ...
    async def _execute_single_analysis_async(self, task: AnalysisTask):
        ...
        try:
            ...
            await self._append_session_event(
                task.task_id,
                "task_started",
                {"symbol": task.symbol, "mode": "single_analysis"},
            )
            ...
            await self._append_session_event(
                task.task_id,
                "task_completed",
                {"symbol": task.symbol, "status": AnalysisStatus.COMPLETED.value},
            )
            await self._update_session_summary(
                task.task_id,
                {
            ...
            await self._append_session_event(
                task.task_id,
                "task_failed",
                {"symbol": task.symbol, "error": str(e)},
            )
            await self._update_session_summary(
                task.task_id,
                {
    ...
    async def submit_single_analysis(
        ...
        try:
            ...
            await self._append_session_event(
                task_id,
                "task_submitted",
                {"symbol": stock_symbol, "mode": "single_analysis"},
            )
            await self._update_session_summary(
                task_id,
                {
    ...
    async def execute_analysis_task(
        ...
        try:
            ...
            await self._append_session_event(
                task.task_id,
                "task_started",
                {"symbol": task.symbol, "mode": "worker_analysis"},
            )
            await self._update_session_summary(
                task.task_id,
                {
            ...
            async def record_graph_event(message: str):
                await self._append_session_event(
                    task.task_id,
                    "graph_progress",
                    {"symbol": task.symbol, "message": message},
                )

            def graph_progress_callback(message: str):
                try:
                    loop = asyncio.get_running_loop()
                    loop.create_task(record_graph_event(message))
                except RuntimeError:
                    logger.debug(f"无法在当前上下文记录图进度事件: {task.task_id}")
