...
async def renew_visibility_timeout(r: Redis, task_id: str, worker_id: str, visibility_timeout: int) -> bool:
    """续期可见性超时，仅允许当前持有租约的 worker 续期。"""
    timeout_key = VISIBILITY_TIMEOUT_PREFIX + task_id
    timeout_data = await r.hgetall(timeout_key)
    if not timeout_data:
        return False

    if timeout_data.get("worker_id") != worker_id:
        return False

    await set_visibility_timeout(r, task_id, worker_id, visibility_timeout)
    return True
